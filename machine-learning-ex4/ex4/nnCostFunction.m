...
x1 = X; % (m 400)
a1 = [ones(m, 1) x1]; % (m 401)
x2 = sigmoid(a1 * Theta1'); % (m 25)
a2 = [ones(m, 1) x2]; % (m 26)
x3 = sigmoid(a2 * Theta2'); % (m 10)

h_theta = x3; % (m 10)

Y = (y == (1:num_labels));

J = - (1.0 / m) * (sum(sum(Y .* log(h_theta))) + sum(sum((1 - Y) .* log(1 - h_theta))));

theta1 = Theta1(:, 2:end);
theta2 = Theta2(:, 2:end);
J += lambda * (0.5 / m) * (sum(sum(theta1 .* theta1)) + sum(sum(theta2 .* theta2)));
...
for t = 1:m
    ...
    d2 = (d3 * Theta2(:, 2:end)) .* sigmoidGradient(z2); % (1 10) * (10 25) .* (1 25)
